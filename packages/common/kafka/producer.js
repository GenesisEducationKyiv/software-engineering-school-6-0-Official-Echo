 ...
 * If Kafka is unavailable publish() catches the error, logs a warning, and returns without throwing.
 * `onPublished`/`onPublishError` let each service hook in its own metrics
 * (e.g. Prometheus counters) without duplicating the connect/publish logic.
 *
 * @param {{
 *   producer?: import("kafkajs").Producer,
 *   onPublished?: (type: string) => void,
 *   onPublishError?: (type: string) => void,
 * }} [options]
 * @returns {{ publish: Function, connect: Function, disconnect: Function }}
 */
export function createProducer({ producer, onPublished, onPublishError } = {}) {
	const kafkaProducer =
